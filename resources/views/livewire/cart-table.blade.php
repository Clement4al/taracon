                    ...
                    <tr>
                        <th scope="row">
                            <ul class="cart_list d-block d-xl-flex">
                                <li class="ps-1 ps-sm-4 pe-1 pe-sm-4"><a href="#"><img src="{{ $product->images->first()?->thumbnail }}" alt="cart1.png"></a></li>
                        ...
                        <td>
                            <form action="{{ route('api.products.cart.update', $product) }}" @submit.prevent="$submit()" x-data="{ quantity: {{ $product->quantity }} }" @change="$dispatch('submit')" @then="$dispatch('cart-updated')" data-quietly>
                                @method('PUT')
                                <div class="cart_btn">
                        ...
                        <td class="">
                            <form action="{{ route('api.products.cart.destroy', $product) }}" x-data @submit.prevent="$submit()" data-confirm method="POST" @then="$dispatch('cart-updated')">
                                @method('delete')
    ...
    <div class="col-lg-4 col-xl-3">
        <div class="order_sidebar_widget style2">
            ...
            <div class="ui_kit_button payment_widget_btn ">
                <a href="{{ route('checkout') }}" type="button" class="btn btn-thm btn-block mb-4">Proceed to checkout</a>
            </div>
        </div>
    </div>
</div>
