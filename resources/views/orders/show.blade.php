                ...
                <div id="kt_app_content_container" class="app-container  container-xxl ">
                    <div class="d-flex flex-column gap-7 gap-lg-10">
                        <div class="d-flex flex-wrap flex-stack gap-5 gap-lg-10">
                            ...
                            @can('delete', $order)
                                <form x-data @submit.prevent="$submit()" action="{{ route('api.orders.destroy', $order) }}" data-confirm>
                                    @method('delete')
                                            ...
                                            <div class="table-responsive">
                                                <table class="table align-middle table-row-bordered mb-0 fs-6 gy-5 min-w-300px">
                                                    <tbody class="fw-semibold text-gray-600">
                                                        ...
                                                        @if($order->author && $order->user->isNot($order->author))
                                                            <tr>
                                                                <td class="text-muted">
                                                                    <div class="d-flex align-items-center">
                                                                        <span class="svg-icon svg-icon-2 me-2">
                                                                ...
                                                                <td class="fw-bold text-end">
                                                                    <div class="d-flex align-items-center justify-content-end">
                                                                        <div class="symbol symbol-circle symbol-25px overflow-hidden me-3">
                                                                            <a href="{{ route('users.show', $order->author) }}">
                                                                                <div class="symbol-label">
                                                                                    <img src="{{ $order->author->photo }}"
                                                                                         alt="Dan Wilson" class="w-100"/>
                                                                                </div>
                                                                            </a>
                                                                        </div>
                                                                        <a href="{{ route('users.show', $order->author) }}" class="text-gray-600 text-hover-primary">
                                                                            {{ $order->author->name }}
                                                                        </a>
                                                                    </div>
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <div class="d-flex align-items-center">
                                                                        <a href="{{ route('products.edit', $product) }}" class="symbol symbol-50px">
                                                                            <span class="symbol-label" style="background-image:url({{ $product->images->first()?->medium }});"></span>
                                                                        </a>
                                                                        <div class="ms-5">
                                    ...
                                    <div class="d-flex flex-column gap-7 gap-lg-10">
                                        <div class="card card-flush py-4 flex-row-fluid">
                                            ...
                                            <div class="card-body pt-0">
                                                ...
                                                >
                                                    <form
                                                        @submit.prevent="$submit()" action="{{ route('api.orders.dispatch.store', $order) }}"
                                                        @then="dispatched = true; location.reload()"
                                                    >
                                                        <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                                                            <thead>
                                                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                                            ...
                                                            @foreach($order->products as $product)
                                                                <tr>
                                                                    ...
                                                                    <td class="mw-400px">
                                                                        ...
                                                                        @else
                                                                            <input value="{{ $product->pivot->location?->name }}"  class="form-control" :disabled="dispatched" x-show="dispatched">
                                                                            <select class="form-select" aria-label="location" required
                                                                                    name="products[{{ $product->id }}][location_id]"
                                                                                    :disabled="dispatched" x-show="! dispatched"
                                                                            >
                                                    ...
                                                    @if($order->isNotDelivered())
                                                        <div class="text-end" x-show="dispatched">
                                                            ...
                                                            <form x-data action="{{ route('api.orders.update', $order) }}" @submit.prevent="$submit()" class="d-inline" @then="location.reload()">
                                                                @method('PUT')
                                                    ...
                                                    <tbody class="fw-semibold text-gray-600">
                                                        @foreach($order->paidTransactions as $transaction)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <a href="{{ route('users.show', $transaction->author) }}" class="text-gray-600 text-hover-primary">{{ $transaction->author->name }}</a>
                                                                </td>
                                                                <td>
                                                                    @can('delete', $transaction)
                                                                        <form @submit.prevent="$submit()" action="{{ route('api.transactions.destroy', $transaction) }}" data-confirm="Are you sure you want to cancel this transaction?" x-data @then="location.reload()">
                                                                            @method('delete')
